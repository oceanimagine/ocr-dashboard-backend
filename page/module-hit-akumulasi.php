...
<table class="table table-bordered" id="table_universitas" style="margin-bottom: 0px;">
    ...
    for($i = $begin; $i <= $end; $i->modify('+1 day')){
        $query_jumlah_ktp = mysqli_query($connect, "select count(id_document) jumlah_hit_ktp from tbl_id_document_collection_log_backup where tipe_document = 'KTP' and DATE_FORMAT(timestamp, '%Y-%m-%d') = '".$i->format("Y-m-d")."'");
        $hasil_jumlah_ktp = mysqli_fetch_array($query_jumlah_ktp);
        $query_jumlah_ijazah = mysqli_query($connect, "select count(id_document) jumlah_hit_ijazah from tbl_id_document_collection_log_backup where tipe_document = 'IJAZAH' and DATE_FORMAT(timestamp, '%Y-%m-%d') = '".$i->format("Y-m-d")."'");
        $hasil_jumlah_ijazah = mysqli_fetch_array($query_jumlah_ijazah);
        $query_jumlah_all = mysqli_query($connect, "select count(id_document) jumlah_hit_all from tbl_id_document_collection_log_backup where DATE_FORMAT(timestamp, '%Y-%m-%d') = '".$i->format("Y-m-d")."'");
