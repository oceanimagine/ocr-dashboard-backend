...
<div class="row">
    ...
    <div class="col-sm-4">
        <input onkeyup="cari(this.value);" type="text" class="form-control" placeholder="Cari Sertifikasi Lain" name="cari_sertifikasi_tambahan" id="cari_sertifikasi_tambahan" value="<?php echo $s; ?>">
    </div> 
</div>
<br />
<div id="hasil_sertifikasi_tambahan">
...
<table class="table table-bordered" id="table_sertifikasi_tambahan" style="margin-bottom: 0px;">
    ...
    <tbody>
        ...
        $query_jumlah_sertifikasi_tambahan = mysqli_query($connect, "
            SELECT a.`id`, a.`nama_pelamar`, b.file_sertifikasi, b.judul_sertifikasi, b.id id_sertifikat FROM `tbl_pelamar_master` a, `tbl_serifikasi_tambahan` b where a.`id` = b.id_pelamar and a.id = '".$id_pelamar."'
        " . $search);
        ...
        $query_sertifikasi_tambahan = mysqli_query($connect, "
            SELECT a.`id`, a.`nama_pelamar`, b.file_sertifikasi, b.judul_sertifikasi, b.id id_sertifikat FROM `tbl_pelamar_master` a, `tbl_serifikasi_tambahan` b where a.`id` = b.id_pelamar and a.id = '".$id_pelamar."'" . $search . " order by b.id_pelamar asc limit $start, $batas_data");
...
function cari(string_search){
    ...
    xmlhttp.onreadystatechange = function(){
        if(this.readyState === 4 && this.status === 200){
            var hasil_sertifikasi_tambahan = document.getElementById("hasil_sertifikasi_tambahan");
            hasil_sertifikasi_tambahan.innerHTML = this.responseText;
            load_td();
        }
    };
    xmlhttp.open("GET","ajax/get_pelamar_other_cert.php?id=<?php echo $id_pelamar; ?>&q=" + encodeURI(string_search));
    xmlhttp.send(null);
}
